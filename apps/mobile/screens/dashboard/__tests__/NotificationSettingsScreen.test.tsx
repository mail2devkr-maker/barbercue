...
it('shows every category ON for a user who never configured anything (default ON)', async () => {
  await mount();
  expect(switches()).toHaveLength(ALL_NOTIFICATION_CATEGORIES.length * 2); // push + in-app each
...
it('turning a push toggle OFF saves exactly that category+channel to the server', async () => {
  await mount();
  await act(async () => switchFor('ARRIVAL_ALERTS', 'PUSH').props.onValueChange(false));
  await flush();
  expect(putCalls()).toEqual([{ category: 'ARRIVAL_ALERTS', channel: 'PUSH', enabled: false }]);
  expect(switchFor('ARRIVAL_ALERTS', 'PUSH').props.value).toBe(false);
  // nothing else moved
...
it('reverts the toggle and says so when the save fails - it never shows a state the server did not store', async () => {
  ...
  await act(async () => switchFor('ARRIVAL_ALERTS', 'PUSH').props.onValueChange(false));
  await flush();
  expect(switchFor('ARRIVAL_ALERTS', 'PUSH').props.value).toBe(true);
...
it('explains that the full-screen arrival prompt still appears while the app is open', async () => {
  await mount();
  expect(screenText()).toContain(t.notificationSettingsArrivalNote);
});
...
it('shows a retry when the settings cannot be loaded', async () => {
  ...
  await flush();
  expect(switches()).toHaveLength(ALL_NOTIFICATION_CATEGORIES.length * 2);
});
