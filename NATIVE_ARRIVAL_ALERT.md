# Native Android arrival alert (mandatory owner arrival prompt)
...
At T-5 minutes before an appointment the backend asks the owner "Has the customer arrived?". That decision is mandatory shop operations, so it must reach the owner whether FastQue is open, backgrounded, killed, or the phone is locked. The alarm-like experience is:
...
| Unlocked, app in background or killed | A heads-up notification (sound + vibration) with **Arrived / Not arrived / Remind me in 2 minutes**; Android does not launch a full-screen activity while the phone is in active use. |
...
- `USE_FULL_SCREEN_INTENT` is auto-granted on Android 14+ only to apps whose core functionality is calling or alarms. **FastQue is not such an app**, so it must be declared honestly as a non-core use: Play Console -> App content -> "Full-screen intent" declaration, use case "time-critical operational confirmation for shop owners". Users grant the special access manually (Settings -> Apps -> FastQue -> Allow full-screen notifications); FastQue prompts for it (Settings -> Notifications readiness card, `ACTION_MANAGE_APP_USE_FULL_SCREEN_INTENT`) and degrades to a heads-up notification when it is not granted.
...
5. The audience is the salon owner (`salon.ownerUserId`); staff devices are not sent this alert today.
